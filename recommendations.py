...
#从反映偏好的字典中返回最为匹配者
#返回结果的个数和相似度函数均为可选函数
def topMatches(prefs,person,n=5,similarity=sim_pearson):       #后面两个为可变参数，5和sim_pearson为默认值，仅当函数只有两个实参输入时有用
	scores=[(similarity(prefs,person,other),other) for other in prefs if other!=person]
	#调用时，例如：recommendations.topMatches(recommendations.critics,'Toby',n=2,similarity=recommendations.sim_distance)
